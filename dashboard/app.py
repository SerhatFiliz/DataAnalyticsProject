...
# ==========================================
# TAB 3: DATASET ANALYSIS (EDA) - STATIC
# ==========================================
with tab_eda:
    ...
    if raw_static:
        ...
        c1.metric("Analyzed Records", f"{len(df_raw):,}")
        c2.metric("Mean Sales", f"{df_raw['sales'].mean():.2f}")
        c3.metric("Zero Sales count", f"{(df_raw['sales']==0).sum()}")
        c4.metric("Unique Stores", f"{df_raw['store_nbr'].nunique()}")
        st.divider()
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("##### 📉 Sales Distribution (Log Scale)")
            fig_dist = px.histogram(df_raw, x="sales", nbins=50, log_y=True, color_discrete_sequence=['#636EFA'])
...
# ==========================================
# PREPARE LIVE PLACEHOLDERS (STATIC)
# ==========================================
with tab_live:
...
with tab_eval:
    ...
    with col_feat:
        ...
        st.caption("Correlation of features with Actual Sales.")
...
def calculate_smart_stats(df_in):
    ...
    if len(valid) < 50 or valid['cluster_id'].nunique() < 3: return None
...
while True:
    try:
        ...
        if preds:
            ...
            # --- FORCE NUMERIC & CLEAN ---
            numeric_cols = ['sales', 'actual_sales', 'predicted_sales', 'onpromotion', 'store_nbr', 'cluster_id']
            ...
            if st.session_state['cluster_table'] is not None:
                ...
                if 'cluster_name' in df_disp.columns:
                    ...
                    fig_pie = px.pie(pie_data, values='Count', names='Segment', color='Segment', color_discrete_map=colors, hole=0.4)
                    ...
                    fig_scat = px.scatter(df_disp, x='store_nbr', y='predicted_sales', color='cluster_name',
                                          labels={'store_nbr': 'Store ID', 'predicted_sales': 'Predicted'}, color_discrete_map=colors)
            ...
            if is_anomaly:
                ...
                if not st.session_state['anomaly_log'] or st.session_state['anomaly_log'][0]['Trans. ID'] != str(last.get('id')):
                    ...
                    st.session_state['anomaly_log'] = st.session_state['anomaly_log'][:5]
            else:
            ...
            if st.session_state['anomaly_log']:
                ...
                ph_anomaly_table.info("No anomalies detected yet.")
            ...
            # --- MODEL DIAGNOSTICS (ENHANCED FEATURE IMPACT) ---
            if len(df_disp) > 10:
                ...
                ph_eval_r2.metric("Live R² (Accuracy)", f"{live_r2:.2f}", help="1.0 is perfect.")
                ph_eval_mae.metric("Live MAE (Error)", f"{live_mae:.1f}", help="Average error.")
                ph_eval_rmse.metric("Live RMSE (Penalty)", f"{live_rmse:.1f}", help="Large error penalty.")
                
                # --- FEATURE IMPACT FIX (INCLUDING FAMILY) ---
                # We need to correlate 'family' too, so we use codes
                corr_prep = df_disp[['actual_sales', 'onpromotion', 'store_nbr', 'cluster_id', 'family']].copy()
                
                # Convert 'family' text to numeric codes just for correlation
                corr_prep['family_code'] = corr_prep['family'].astype('category').cat.codes
                
                # Drop original text family
                ...
                if corr_prep.std().min() > 0: # Avoid NaN
                    ...
                    ph_feat_imp.info("Waiting for data variance...")
